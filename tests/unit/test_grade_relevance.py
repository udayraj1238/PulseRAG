...
@pytest.mark.asyncio
async def test_low_confidence_excluded(mock_llm):
    mock_llm.ainvoke.return_value.content = '{"relevant": true, "confidence": 0.5, "reason": "maybe"}'
    state = await grade_relevance_node(make_state_with_chunks(1))
    assert state["relevant_chunk_count"] == 0
